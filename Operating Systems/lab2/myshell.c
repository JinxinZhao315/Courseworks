...
void sigint_handler()
{
    ...
    for (int i = 0; i < free_index; i++)
    {
        if (PCB_arr[i].status == 2) // PCB_arr[i].pid == waiting_pid &&
        {
            running_pid = PCB_arr[i].pid;
            kill(running_pid, SIGINT);
            printf("[%d] interrupted\n", running_pid);
            PCB_arr[i].status = 1;
            PCB_arr[i].exitCode = 2;
            break;
        }
    }
}
        ...
        else
        {
            ...
            if (access(command, F_OK) == -1)
            {
                ...
                else if (strcmp(command, "fg") == 0)
                {
                    ...
                    {
                        int cont_pid = atoi(curr_command[1]);
                        for (int i = 0; i < free_index; i++)
                        {
                            if (PCB_arr[i].pid == cont_pid)
                            {
                                if (PCB_arr[i].status == 4)
                                {
                                    kill(cont_pid, SIGCONT);
                                    printf("[%d] resumed\n", cont_pid);
                                    PCB_arr[i].status = 2;
                                    int status;
                                    // waiting_pid = cont_pid;
                                    waitpid(cont_pid, &status, WUNTRACED);
                                    // waiting_pid = -1;
                                    if (WIFEXITED(status))
                                    {
                                        int exit_code = WEXITSTATUS(status);
...
void my_quit(void)
{
    ...
    for (int i = 0; i < free_index; i++)
    {
        if (PCB_arr[i].status == 2 || PCB_arr[i].status == 4)
        {
            printf("Killing [%d]\n", PCB_arr[i].pid);
